...
| "I'll just skip installing Gemini in this project to avoid triple final-pass" | Codex-only-plus-Plan mode still requires Plan-subagent on every final-pass, so you only save one of three calls (Gemini's), while inheriting the mandatory `Gemini SKIPPED ...; Plan-subagent compensating as second voice` disclosure on every report. Choosing it deliberately to escape discipline is observable in the scorecard and report trail. The right move when Gemini is genuinely unavailable: accept Codex-only-plus-Plan and disclose. The wrong move: install-skip as policy laundering. |
| "Reports don't really need the `Gemini SKIPPED` line, the user can see Gemini isn't installed" | Wrong. The disclosure is non-optional in Codex-only-plus-Plan mode. It's how you and the user track when quasi-dual (Codex + Plan-subagent) was used vs. triple, for retrospective calibration of misses. |
| "Plan-subagent is optional, so I'll skip it on tie-break to save tokens" | Tie-break is one of several v2.0 cases where Plan-subagent is **mandatory** (alongside Full-mode final-pass, Codex-replacement, Gemini-drift replacement, Codex-only-plus-Plan final-pass), not optional. Picking a winner between two confidently-disagreeing models without a fresh-context third voice is exactly the failure mode this role exists to prevent. |
...
| "I only did post-review fixes after the final-pass — no new final-pass needed for those edits" | **Any edit to the final artifact after a final-pass invalidates that pass.** Final-pass is bound to a specific artifact version (path + commit SHA / file hash / frontmatter version). Even one-paragraph edits — typo fixes, wording tweaks, formatting — produce a new artifact, which needs a new final-pass dispatch. For impl-verify final-passes specifically: edits to **the implementation under verification** (not just the spec/plan) also invalidate the pass — the consultants reviewed implementation state X; if you commit a fix, you're now at X+1 and need a re-verify. The only edits that don't invalidate are explicitly non-artifact metadata (e.g., file rename without content change). |
| "I dispatched all three but Plan-subagent's findings are just hallucinations because it lacks the iter-1..N-1 context we had" | Compliance theater: dispatching to satisfy the policy while systematically dismissing the third channel's output undermines the entire point. Plan-subagent's fresh-context is a *feature* — it catches what persistent-context reviewers normalized away. Treat its BLOCKERs with the same severity as Codex/Gemini BLOCKERs. If you genuinely think a Plan-subagent finding is wrong, verify against the codebase (cite file:line) rather than dismissing it as context-less. |
| "All three reviewed the same topic, just slightly different prompts / different versions of the artifact" | Final-pass requires **same artifact version + same final-pass question text** for all three. Different prompts produce different findings that can't be compared on convergence/divergence axis. If you have to re-dispatch one because the prompt was off, re-dispatch all three on the corrected prompt — partial triples don't satisfy the policy. |
| "Impl verification isn't really a final-pass, the policy table only covers spec/plan" | The policy table includes "Verify implementation against spec/plan" as a triple-mandatory row alongside spec-final and plan-final. Impl-verify IS a final-pass event (verifying a built thing matches its spec). Don't read past the row. |
| "Sequential dispatch is technically still the same pass, just spread over multiple turns" | The same-orchestration-turn rule exists because Plan-subagent runs in the SAME assistant message as the bash backgrounds; splitting destroys parallelism and inflates wall-clock cost. The "rare exception" for serial dispatch (e.g., needing wrapper output to compose Plan-subagent prompt) requires explicit disclosure in the report. Default-serial = policy laundering. |
...
```bash
# Compose question files: same artifact version + same review questions
# for all three; tool-specific preamble/scaffold may differ (Gemini's read-only prefix,
# Plan-subagent's "critical files to grep" section). Materially different prompts
# break convergence/divergence comparison — don't paper over with "near-identical".
cat > /tmp/codex_question.txt  <<'EOF' ...
...
The `Gemini SKIPPED ...; Plan-subagent compensating as second voice` line is the disclosure cost of Codex-only-plus-Plan mode. Don't bury it; surface it explicitly so future-you and the user can audit when quasi-dual (Codex + Plan-subagent) discipline was used instead of triple.
...
| Plan-subagent drift (answers wrong-artifact, addresses wrong review phase, hallucinates BLOCKERs that don't match the actual code) | Thin/wrong prompt; wrong commit-SHA/version reference in prompt; subagent took shortcuts. Note: citing **extra** real files beyond the grep-list is NOT drift — that's often a useful catch. Drift means the answer is unmoored from the actual artifact. | First, distinguish cause: (a) if your Plan-subagent prompt was materially defective (wrong artifact, wrong commit, missing critical context), the WHOLE triple is suspect because the same defect would have affected the others too — re-compose the question and re-dispatch all three; (b) if the prompt was equivalent to Codex/Gemini's prompts and only Plan-subagent drifted, re-dispatch ONLY Plan-subagent with a tighter scaffold (quote artifact text inline, explicit grep-list). If the second Plan-subagent dispatch also drifts, escalate to user — the prompt template needs rework. Don't paper over with "Plan-subagent had no context" rationalization. |
